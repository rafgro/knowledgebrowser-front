        ...
        (error, response, body) => {
          ...
          Promise.map(toAsk, (query) => {
            ...
            whatWeHave.forEach((one) => {
              ...
              toWrite += `<h3 style="padding-bottom:0.25rem${up}"><a href="https://knowledgebrowser.org/preprints/search?q=${one[1].replace(/ /g, '+')}">${one[1].substring(0, 1).toUpperCase() + one[1].substring(1)}</a></h3><small style="padding-bottom:1rem">${queries[iterator].n} preprints</small>`; // head
              let itsResults = null;
              try {
            ...
            fs.writeFile('./views/preprint-weekfeed.handlebars', toWrite, (err) => {
              if (err) reject(err);
              resolve('Successfully written week feed.');
            });
          }).catch(e => reject(e));
        });
    }
