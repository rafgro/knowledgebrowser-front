            ...
            if( error ) {
                ...
                if( results != null ) {
                    if( results.hasOwnProperty('message') ) { reject("No results."); }
                        ...
                        else { paginationObj.prev_link = 'https://knowledgebrowser.org/preprints/search?q='+origQuery+'&offset='+(intOffset-10)+whatSort; }
                        if( intOffset+10 >= allresults ) { paginationObj.next_link = '#'; paginationObj.next_activity = 'disabled'; }
                        else { paginationObj.next_link = 'https://knowledgebrowser.org/preprints/search?q='+origQuery+'&offset='+(intOffset+10)+whatSort; }
                        let maxButtons = 0;
